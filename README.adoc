...
package "Repository Layer" {
  [Reactive Repositories] as Repositories
}
...
Client -right-> Auth : Authenticate
Auth -down-> API : JWT Token
API -down-> Services : Invoke
Services -down-> Repositories : Access data
Repositories -down-> DB : R2DBC
Scheduler -right-> Services : Execute jobs
@enduml
....
...
* link:docs/architecture.puml[System Architecture]
* link:docs/domain.puml[Domain Model]
* link:docs/transfer-sequence.puml[Money Transfer Sequence]
...
* `POST /scheduled-transfers` — Create a scheduled transfer
* `GET /scheduled-transfers/{id}` — Get transfer by ID
* `GET /scheduled-transfers/account/+{accountId}+` — Get transfers for an account
